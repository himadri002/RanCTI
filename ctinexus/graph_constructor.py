...
class Merger:
	...
	def _request_openrouter_embeddings(self, model, texts, base_url, api_key):
		...
		payload_model = model.split("/", 1)[-1] if model.lower().startswith("openrouter/") else model
		payload = {"model": payload_model, "input": texts}
		print("[Merger.get_embeddings] request:", payload)
		response = requests.post(url, headers=headers, json=payload, timeout=120)
		response.raise_for_status()
		return response.json()

	def get_embeddings(self, texts):
		"""Get embeddings for multiple texts in a single API call (OpenRouter-only)"""
		startTime = time.time()

		def _openrouter_model(name: str | None) -> str | None:
			if not name:
				return None
			lower = name.lower()
		...
		model_arg = _openrouter_model(getattr(self.config, "embedding_model", None))
		if not model_arg:
			raise ValueError("Embedding model not configured for OpenRouter.")
		...
		self.response = self._request_openrouter_embeddings(model_arg, texts, base_url, api_key)
