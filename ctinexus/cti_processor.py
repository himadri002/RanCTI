...
def preprocessor(result: dict) -> dict:
    ...
    if "ET" in result and isinstance(result["ET"], dict) and "typed_triplets" in result["ET"]:
        typed_triplets = result["ET"]["typed_triplets"]
    elif "tagged_triples" in result:
        typed_triplets = result["tagged_triples"]
    else:
        raise KeyError("Expected either ET.typed_triplets or tagged_triples in the tagging result payload.")

    jsr["EA"]["aligned_triplets"] = typed_triplets
