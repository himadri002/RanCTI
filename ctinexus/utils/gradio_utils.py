...
def get_config(model: str = None, embedding_model: str = None, similarity_threshold: float = 0.6) -> DictConfig:
	...
	def _normalize_override(name: str | None) -> str | None:
		if not name:
			return None
		lname = name.lower()
		if lname.startswith("openrouter/"):
			return name
		if lname.startswith("openai/") or lname.startswith("google/"):
			return f"openrouter/{name}"
		# bare or other prefixes -> force openrouter
		return f"openrouter/{name.split('/',1)[-1]}" if "/" in name else f"openrouter/{name}"

	model = _normalize_override(original_model)
	embedding_model = _normalize_override(original_embedding_model)
	...
	with initialize(version_base="1.2", config_path=CONFIG_PATH):
		...
		overrides.append("provider=openrouter")
...
def run_pipeline(
	...
	try:
		...
		tagged_entities_path = resolve_path("utils", "tagged_entities.json")
		with open(tagged_entities_path, "r", encoding="utf-8") as f:
			tagged_payload = json.load(f)

		def _usage_stub(model_name: str | None) -> dict:
			return {
				"model": model_name.split("/")[-1] if model_name else "external",
		...
		tagging_result = {
			"IE": {
				"triplets": [],
				"response_time": 0.0,
				"model_usage": _usage_stub(ie_model),
			},
			"ET": {
				"typed_triplets": tagged_payload.get("tagged_triples", []),
				"response_time": 0.0,
				"model_usage": _usage_stub(et_model),
			},
		}
...
def process_and_visualize(
	...
	# Force provider dropdown to OpenRouter behavior; safe prefix helper
	def _safe_openrouter(name: str | None) -> str | None:
		if not name:
			return None
		return name if name.startswith("openrouter/") else (f"openrouter/{name}" if "/" not in name else f"openrouter/{name.split('/',1)[-1]}")

	custom_model = _safe_openrouter(custom_model_input)
	custom_embedding_model = _safe_openrouter(custom_embedding_model_input)
